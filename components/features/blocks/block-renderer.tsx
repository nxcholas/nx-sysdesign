...
import {
  User, Smartphone, Monitor, MonitorSmartphone,
  Cpu, Terminal, Blocks, ArrowLeftRight, Cog, Clock,
  Webhook, Globe, Globe2, ShieldAlert, Network, MapPin, Lock,
  KeyRound, BadgeCheck, BellRing, HeartPulse, LayoutDashboard,
  Share2, TrendingUp, Plug, MessageSquare, CreditCard, BarChart3,
  Database,
  type LucideIcon,
} from 'lucide-react';
import {
...
function ServerRackVisual({ size }: { size: 'sm' | 'md' }) {
  ...
  return (
    <div
      aria-hidden="true"
      className={`${containerDim} rounded bg-violet-700 border border-violet-500 flex flex-col items-center justify-center gap-0.5 px-1`}
    >
      {[0, 1, 2].map((i) => (
        <div key={i} className="w-full flex items-center gap-0.5">
          <div className="w-1 h-1 rounded-full bg-green-400" />
          <div className="flex-1 h-0.5 rounded bg-violet-500" />
        </div>
      ))}
    </div>
  );
